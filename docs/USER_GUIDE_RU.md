# LastFrame — краткое руководство

## Запуск

Для portable-версии распакуйте `LastFrame-windows-x64-portable.zip` в каталог без ограничений на запись и запустите `LastFrame.exe`. В installer-версии достаточно запустить установщик. Portable ZIP уже содержит `ffmpeg.exe`, его DLL и Qt runtime.

После запуска окно можно закрыть — LastFrame останется в системном трее. Буфер не запускается автоматически: нажмите «Начать буфер».

## Быстрый сценарий

1. На странице Capture выберите монитор, весь экран или прямоугольную область.
2. При необходимости задайте FPS и выходное разрешение.
3. На странице Video выберите MP4, MKV или WebM и профиль кодирования.
4. Нажмите «Начать буфер».
5. Нажмите «Сохранить клип» или `Ctrl+Shift+F10`.

Клипы по умолчанию сохраняются в `Видео/LastFrame`. Готовый файл появляется только после успешного экспорта и атомарного переименования.

Перед запуском буфера страницы Capture/Video показывают предупреждения capability: например, если ещё не завершена проверка FFmpeg, выбранный encoder или VP9 недоступен, нет WASAPI либо FPS выше частоты монитора. Это предупреждение не скрывает автоматический fallback.

Если capability probe подтверждает `scale_cuda` и NVENC, LastFrame пытается масштабировать видео на GPU. При несовместимости драйвера незавершённый сегмент удаляется, а запись продолжается через обычный worker/software scaler.

## Горячие клавиши

По умолчанию:

- `Ctrl+Shift+F10` — сохранить клип;
- `Ctrl+Shift+F1` — очистить буфер;
- `Ctrl+Shift+F7` — пауза/продолжение;
- `Ctrl+Shift+F5` — старт/остановка буфера.

На странице Hotkeys комбинации можно изменить. Пустое поле отключает действие; дубликаты и занятые Windows-комбинации отклоняются.

## Аудио и ограничения MVP

На странице Audio можно выбрать найденный микрофон и отдельно настроить громкость system/mic. Windows backend использует native DXGI Desktop Duplication через D3D11, затем Windows Graphics Capture, затем FFmpeg `ddagrab`, а при недоступности API — GDI fallback. Native WASAPI loopback и microphone capture сводятся через общий master-clock `AudioMixer` в одну 48 kHz stereo дорожку; при потере источника приложение показывает `[audio_device_lost]` и продолжает с доступным источником или fallback FFmpeg. Mute/unmute микрофона доступен через заданный global hotkey и пункт tray. Device-loss recovery с автоматическим resume и HDR/DRM capability states реализованы в Windows-срезе; native macOS/Linux capture и прямые FFmpeg libraries остаются следующими production-срезами.

Проверка обновлений запускается только вручную на странице Advanced и обращается к GitHub Releases. Телеметрия отсутствует.

На странице Notifications можно отключить системные уведомления и угловой click-through overlay, выбрать язык RU/EN, один из четырёх углов, длительность и прозрачность. Overlay показывает короткие события записи/экспорта, не принимает мышь; Windows запрашивает исключение окна из поддерживаемых capture API.

Язык всего интерфейса переключается на странице Advanced в поле «Язык интерфейса». Изменение применяется сразу, перестраивает окно и меню трея и сохраняется в `settings.json`. Язык уведомлений можно оставить отдельным.

Координаты пользовательской области сохраняются логически и автоматически переводятся в физические пиксели монитора, поэтому региональный захват учитывает масштабирование Windows. На конфигурациях с несколькими мониторами и разным DPI перед релизом нужна ручная проверка каждого дисплея.

На странице Storage можно выбрать каталог клипов только на локальном фиксированном диске, открыть его и очистить временные сегменты/буфер. Завершённые сегменты удерживаются в RAM до мягкого лимита 70%, затем используются с локальным disk spillover.

При блокировке Windows или sleep захват ставится на паузу. После возврата источника приложение продолжает его только при включённом `Auto resume` и после проверки стабильности.

На macOS глобальные хоткеи регистрируются через Carbon Event Hot Keys. Если macOS не выдал разрешение Accessibility/Input Monitoring, LastFrame показывает конфликт и предлагает включить нужное разрешение, а не принимает хоткей молча.
